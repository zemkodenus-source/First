...
def product_detail(request, pk):                               # сторінка ДЕТАЛЬНІШЕ для кожного іх товарів
    ...
    #basket = Basket.objects не потрібно бо данні тут витягувати не будем (корзина)
    ...
    if request.method == 'POST':
        ...
        elif 'add_to_basket' in request.POST:
            # 1. Шукаємо, чи є вже такий товар у цього юзера
            basket_item, created = Basket.objects.get_or_create(
                product=product,
                user=request.user,
                defaults={'quantity': 1}  # якщо створюємо новий, ставимо 1
            )

            if not created:
                # 2. Якщо товар вже був (created=False) — просто збільшуємо кількість
    ...
    return render(request, 'catalog_products/product_detail.html', {'product': product ,
                                                                    'comments': comments ,
                                                                    'form': form ,})